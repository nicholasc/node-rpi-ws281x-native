...
var _loutputBuffer = null;
var _routputBuffer = null;
 ...
 */
ws281x.init = function (numLeds) {
  _isInitialized = true;

  const bufferSize = 4 * numLeds;
  _loutputBuffer = Buffer.alloc(bufferSize);
  _routputBuffer = Buffer.alloc(bufferSize);

  bindings.init(numLeds);
};
 ...
 */
ws281x.render = function (ldata, rdata) {
  if (!_isInitialized) {
    throw new Error("render called before initialization.");
  }

  this.emit("beforeRender", ldata);
  this.emit("beforeRender", rdata);

  if (_indexMapping) {
    ldata = remap(ldata, _indexMapping);
    rdata = remap(rdata, _indexMapping);
  }

  for (var i = 0; i < ldata.length; i++) {
    _loutputBuffer.writeUInt32LE(gammaCorrect(ldata[i]), 4 * i);
    _routputBuffer.writeUInt32LE(gammaCorrect(rdata[i]), 4 * i);
  }

  bindings.render(_loutputBuffer, _routputBuffer);

  this.emit("render", [ldata, rdata]);
};
